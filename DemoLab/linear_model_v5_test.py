...
if __name__ == "__main__":
    ...
    for test_week_num in [3, 10]:
        for is_log in [True, False]:
            ...
            if is_log:
                file_name = "input/log_linear_model_v"+model_version+"_exp_" + str(test_week_num) + "weeks.csv"
            else:
                file_name = "input/linear_model_v"+model_version+"_exp_" + str(test_week_num) + "weeks.csv"
            ...
            with open(file_name, "w") as file:
                ...
                for pid in id_list:
                    ...
                    x[:, 5] = train_data[ahead_period:, 2]  # whether is a special day
                    x[:, 6] = train_data[ahead_period:, 3]  # whether is ahead a special day
                    x[:, 7] = train_data[ahead_period:, 4]  # whether is behind a special day
                    ...
                    test_x[:, 5] = test_data[ahead_period:, 2]  # whether is a special day
                    test_x[:, 6] = test_data[ahead_period:, 3]  # whether is ahead a special day
                    test_x[:, 7] = test_data[ahead_period:, 4]  # whether is behind a special day
